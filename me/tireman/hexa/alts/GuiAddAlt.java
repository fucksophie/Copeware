    ...
    extends Thread {
        ...
        private final void checkAndAddAlt(String username, String password) throws IOException {
            ...
            try {
                ...
                try {
					(new File("alts.cope")).createNewFile();
					System.out.println("Saved" + this.username + " and " + this.password + " with username " + auth.getSelectedProfile().getName());
					Files.write(Paths.get("alts.cope"), (this.username + "|" + this.password + "|"+ auth.getSelectedProfile().getName() + "\n").getBytes(), StandardOpenOption.APPEND);
        ...
        public void run() {
            if (this.password.equals("")) {
                ...
                try {
					(new File("alts.cope")).createNewFile();
					System.out.println("Saved" + this.username + " and " + this.password + "with mask ");
					Files.write(Paths.get("alts.cope"), (this.username + "|" + this.password + "|\n").getBytes(), StandardOpenOption.APPEND);
				} catch (IOException e1) {
					e1.printStackTrace();
				}
                return;
